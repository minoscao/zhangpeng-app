...
- “当地背景”使用短地点标签配合编辑弹窗中的地域线索说明；“产品配色”必须明确只作用于帐篷面料。
...
- 默认生成质量为精细成片，输出视觉基线是真实、精修、大气的商业摄影，不接受插画、平面示意或廉价 3D 渲染质感。
- 亲子场景默认只出现一名与产品受众年龄一致的儿童，采用简单静止动作；面部与帐篷入口处于同一清晰焦平面。
- 场景必须服从单一相机、连续地面、单一消失点和统一光向；一个主地标足以表达地域，不同时堆叠多个大型地标。

## Do's and Don'ts

### Do:

- **Do** 使用真实白底产品图作为产品库和创作参考图的视觉事实。
...
- **Don't** 用场景效果图代替产品底图。
- **Don't** 在创作流程中增加固定产品规格步骤；参考图可以与不同提示词自由组合。
