  ...
  return `<section class="view">
    ...
    <div class="studio-layout">
      <aside class="card control-panel">
        ...
        <div class="control-section"><div class="control-title"><h4>结构类目</h4><span>8 种主流款式</span></div><div class="visual-choice-grid visual-choice-grid--structure">${kidsCategories.map((item, index) => `<button class="visual-choice ${product.category === item.name ? 'is-active' : ''}" data-choice-param="category" data-value="${escapeHtml(item.name)}" aria-pressed="${product.category === item.name}"><img src="${tentImage(['#7467de','#ff8fa4','#68bfa8','#67bfe5'][index % 4], '#ffcf5b', '', item.key)}" alt=""><span>${escapeHtml(item.name)}</span></button>`).join('')}</div></div>
        <div class="control-section"><div class="control-title"><h4>使用场景</h4><span>支持本地图片融合</span></div><div class="visual-choice-grid">${kidsScenes.map((item) => `<button class="visual-choice ${p.scene === item.name ? 'is-active' : ''}" data-choice-param="scene" data-value="${escapeHtml(item.name)}" aria-pressed="${p.scene === item.name}"><img src="${sceneVisual(item.key)}" alt=""><span>${escapeHtml(item.name)}</span></button>`).join('')}<button class="visual-choice visual-choice--upload ${p.scene === '本地上传场景' ? 'is-active' : ''}" data-action="upload-scene" aria-pressed="${p.scene === '本地上传场景'}"><img src="${state.ui.customSceneImage || sceneVisual('upload')}" alt="${state.ui.customSceneImage ? '已上传场景预览' : ''}"><span>${state.ui.customSceneImage ? '更换本地场景' : '上传本地场景'}</span><small>${state.ui.customSceneImage ? '已选择并参与融合' : 'JPG · PNG · WebP'}</small></button></div><input id="scene-upload" type="file" accept="image/png,image/jpeg,image/webp" hidden></div>
        <div class="control-section"><div class="control-title"><h4>材质</h4><span>表面质感与结构</span></div><div class="visual-choice-grid">${kidsMaterials.map((item) => `<button class="visual-choice ${p.material === item.name ? 'is-active' : ''}" data-choice-param="material" data-value="${escapeHtml(item.name)}" aria-pressed="${p.material === item.name}"><img src="${materialVisual(item.key)}" alt=""><span>${escapeHtml(item.short)}</span></button>`).join('')}</div></div>
        <div class="control-section"><div class="control-title"><h4>原创童趣主题</h4><span>通用原创图形</span></div><div class="visual-choice-grid visual-choice-grid--large">${kidsThemes.map((item) => `<button class="visual-choice visual-choice--large ${p.theme === item.name ? 'is-active' : ''}" data-choice-param="theme" data-value="${escapeHtml(item.name)}" aria-pressed="${p.theme === item.name}"><img src="${themeVisual(item.key)}" alt=""><span>${escapeHtml(item.name)}</span></button>`).join('')}</div></div>
        ...
        <div class="control-section"><div class="control-title"><h4>视觉风格</h4><span>控制整体画面语言</span></div><div class="visual-choice-grid visual-choice-grid--large">${kidsStyles.map((item) => `<button class="visual-choice visual-choice--large ${p.style === item.name ? 'is-active' : ''}" data-choice-param="style" data-value="${escapeHtml(item.name)}" aria-pressed="${p.style === item.name}"><img src="${styleVisual(item.key)}" alt=""><span>${escapeHtml(item.name)}</span></button>`).join('')}</div></div>
