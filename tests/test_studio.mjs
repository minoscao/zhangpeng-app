...
run(`for (const target of [seedState, state]) {
  ...
  locationGroup.options.forEach((option) => {
    if (option.selected) {
      option.sceneReferenceImage = 'data:image/jpeg;base64,c3RyZWV0';
      option.sceneReferenceName = option.id + '-street.jpg';
      option.sceneReferenceUpdatedAt = '2026-09-15T00:00:00.000Z';
    }
  });
}`);
...
run(`Object.assign(state.promptGroups[0].options.find((option) => option.id === 'melbourne'), {sceneReferenceImage: 'data:image/jpeg;base64,bWVsYm91cm5l', sceneReferenceName: 'melbourne-street.jpg', sceneReferenceUpdatedAt: '2026-09-15T01:00:00.000Z'});`);
...
assert.match(melbournePrompt, /必须彻底移除图1旧背景/);
assert.match(melbournePrompt, /图2为用户核验的 Google Maps 实景/);
assert.match(melbournePrompt, /两图职责不可交换/);
assert.match(melbournePrompt, /实景证据】已绑定/);
assert.equal(run('buildCombinations()[0].sceneReferenceId'), 'melbourne');
assert.equal(run('buildCombinations()[0].sceneReferenceReady'), true);
assert.match(run("googleMapsSearchUrl(state.promptGroups[0].options.find((option) => option.id === 'melbourne'))"), /^https:\/\/www\.google\.com\/maps\/search\/\?api=1&query=/);
run("delete state.promptGroups[0].options.find((option) => option.id === 'melbourne').sceneReferenceImage; comparisonRequested = false; confirmedPromptReviews = {batch: null, comparison: null}; preparePromptReview(); confirmCurrentPromptReview(true);");
assert.equal(run('hasConfirmedPromptReview(false)'), false);
assert.equal(run('selectedLocationsMissingEvidence().length'), 1);
assert.equal(run('state.ui.promptDialogGroupId'), 'group-location');
run("Object.assign(state.promptGroups[0].options.find((option) => option.id === 'melbourne'), {sceneReferenceImage: 'data:image/jpeg;base64,bWVsYm91cm5l', sceneReferenceName: 'melbourne-street.jpg', sceneReferenceUpdatedAt: '2026-09-15T01:00:00.000Z'}); state.ui.promptDialogGroupId = ''; promptReview = null; promptReviewError = '';");
...
assert.ok(run('promptReview.entries[0].prompt.length') <= 2000);
...
assert.equal(run('state.schemaVersion'), 15);
...
assert.match(run('renderPromptDialog()'), /添加并补全文字规则/);
assert.match(run('renderPromptDialog()'), /data-region-prompt/);
assert.match(run('renderPromptDialog()'), /在 Google Maps 核验/);
assert.match(run('renderPromptDialog()'), /data-scene-reference/);
